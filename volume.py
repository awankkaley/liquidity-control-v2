...
class Volume(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.label1 = Label(master, text="Random min quantity (USDT) :", pady=3)
        self.label1.grid(row=2, column=0, ipadx=20, sticky=E)
        self.min_usdt = Entry(master)
        self.min_usdt.grid(row=2, column=1, ipadx=20)

        self.label2 = Label(master, text="Random max quantity (USDT) : ", pady=3 )
        ...
        self.label4 = Label(master, text="Delay in seconds (Ex: 5.5) : ", pady=3)
        self.label4.grid(row=5, column=0, ipadx=20, sticky=E)
        self.delay = Entry(master)
        self.delay.grid(row=5, column=1, ipadx=20)

        self.label5 = Label(master, text="Maximum limit price (USDT): : ", pady=3)
        ...
        self.label6 = Label(master, text="Minimum limit price (USDT): : ", pady=3)
        ...
        frame = Frame(master)  
        Radiobutton(frame, text="Sell First", variable=self.priority, value=1).pack(side = RIGHT )
        Radiobutton(frame, text="Buy First", variable=self.priority, value=2).pack(side = RIGHT )
        frame.grid(row=8, column=1, sticky=W)

        self.proses = Button(master, text="START", command=lambda:threading.Thread(target=self.start_process()).start())
    ...
    def start_process(self):
        ...
        if self.validation() == True:
            ...
            self.start(float(delay), float(min_price_difference), float(min_usdt), float(max_usdt), float(max_limit_price), float(min_limit_price), self.market, int(self.quantity_decimals), int(self.price_decimals), str(self.api_key), str(self.private_key), str(self.exchange), int(priority))
